...
import ModalRejectAppointment from '@pages/Admin/Appointment/components/ModalRejectAppointment';
...
export default function DoctorManageAppointment() {
  const [page, setPage] = useState(1);
  const [limit, setLimit] = useState(10);
  const [statusAppointment, setStatusAppointment] = useState<
    EAppointmentStatus | 'all'
  >('all');
  const { id: doctorId } = useSelector((state: IRootState) => state.auth.user!);
