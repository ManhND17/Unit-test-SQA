...
import { Check, Eye, GitPullRequest, Pencil, Trash, X } from 'lucide-react';
...
import ModalRejectArticle from './components/ModalRejectArticle';
...
export default function AdminManageArticles() {
  ...
  const tableRows: (ArticleRow & { original: IHealthArticle })[] =
    getArticles?.data?.data?.map((item: IHealthArticle) => ({
      ...
      status: item?.status,
      rejectReason: item?.reasonReject,
    ...
    },
    {
      ...
      renderCell: ({ row }) => (
        <div className="flex justify-start items-center gap-2 flex-wrap">
            ...
            <>
              <IconButton
                ...
                onClick={() => {
                  ...
                  navigate(`/admin/articles/create-edit?id=${row.original.id}`);
                }}
              >
                <Pencil size={20} />
              </IconButton>
          ...
          {row.status === EArticleStatus.PENDING_REVIEW && (
            <>
              <IconButton
                size="small"
  ...
  const handleCreateArticle = () => {
    ...
    navigate('/admin/articles/create-edit');
  };
  ...
  return (
    <>
        ...
        <div className="flex gap-5 flex-wrap mb-4">
          ...
          <div>
            ...
            <CommonSelect
              values={[
                { name: 'Tất cả', id: 'all' },
                { name: 'Đã xuất bản', id: 'published' },
                { name: 'Đang chờ duyệt', id: 'pending_review' },
                { name: 'Chỉnh sửa lại', id: 'reedited' },
              ]}
              value={statusArticle}
              onChange={(selected) =>
      ...
      <ModalRequestEdit
        open={openRequestEditModal}
        onClose={() => setOpenRequestEditModal(false)}
      ...
      />
      <CommonModal
        title={`Duyệt bài viết: ${selectedArticle?.title || ''}`}
        open={openApproveModal}
        onClose={() => setOpenApproveModal(false)}
      >
        <>
          <div>Bạn có chắc chắn muốn duyệt bài viết này không?</div>
          <div className="flex justify-end items-center gap-4 mt-6">
            <CommonButton
              text="Hủy"
            ...
            <CommonButton
              ...
              onClick={() => {
                ...
                ApiArticle.approveArticle(selectedArticle.id)
                  .then(() => {
                  ...
                  .catch(() => {
                    toast.error('Duyệt bài viết thất bại. Vui lòng thử lại.');
                  });
              }}
            />
          </div>
